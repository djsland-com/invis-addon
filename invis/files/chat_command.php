...
else if($command[0] == '/invis' && boomAllow(10)){
	//Custom Code by DJ//
	$result = $mysqli->query("SELECT user_status FROM boom_users WHERE user_id = 1");
	$row = $result->fetch_assoc();
	if ($row["user_status"] == 1 || $row["user_status"] == 2 || $row["user_status"] == 3) {
		$mysqli->store_result();
		$mysqli->query("UPDATE boom_users SET user_status = 6 WHERE user_id = 1");
		echo boomCode(1, array('text'=> statusTitle(6), 'icon'=> newStatusIcon(6)));
		die();
	} else if ($row["user_status"] == 6) {
		$mysqli->store_result();
		$mysqli->query("UPDATE boom_users SET user_status = 1 WHERE user_id = 1");
		echo boomCode(1, array('text'=> statusTitle(1), 'icon'=> newStatusIcon(1)));
		die();
	}
}
else {
